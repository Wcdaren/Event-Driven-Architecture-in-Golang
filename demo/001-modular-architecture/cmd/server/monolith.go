...
func (a *app) waitForWeb(ctx context.Context) error {
	...
	group.Go(func() error {
		fmt.Println("web server listening on", a.cfg.Web.Address())
		defer fmt.Println("web server stopped")
...
func (a *app) waitForRPC(ctx context.Context) error {
	...
	group.Go(func() error {
		fmt.Println("rpc server listening on", a.cfg.Rpc.Address())
	...
	group.Go(func() error {
		...
		case <-timeout.C:
			// Force it to stop
			a.RPC().Stop()
			return fmt.Errorf("rpc server shutdown timeout")
		case <-stopped:
			return nil
		}
	})
